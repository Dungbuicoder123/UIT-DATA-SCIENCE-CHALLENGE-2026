...
def preprocess_queries(
    ...
    """
    Trả về:
        query_ids         : List[str]  — question IDs
        processed_queries : List[str]  — đã tách từ (cho BM25)
        raw_queries       : List[str]  — câu gốc đã clean (cho Dense)
    """
    query_ids         = []
    processed_queries = []
    raw_queries       = []
...
def run_predict(force_rebuild: bool = False) -> None:
    """
    Chạy pipeline đầy đủ trên tập test và tạo submission.zip.
    """
    print("\n" + "=" * 60)
    ...
    # Precompute query vectors (batch để nhanh hơn)
    query_vecs = None
...
def run_eval(force_rebuild: bool = False) -> Dict:
    """
    Đánh giá pipeline trên tập train.json (có ground truth).
    Hữu ích để điều chỉnh hyperparameters trước khi nộp bài.
    """
    print("\n" + "=" * 60)
    ...
    # Load train queries (có ground truth)
    ...
    for q in train_queries:
        ...
        # Đảm bảo ground truth là list of strings
    ...
    # Lưu kết quả đánh giá chi tiết ra file
...
# ---------------------------------------------------------------------------
# Mode: TUNE (grid search hyperparameters đơn giản)
# ---------------------------------------------------------------------------

def run_tune(force_rebuild: bool = False) -> None:
    """
    Grid search nhẹ trên tập train để tìm tổ hợp hyperparameter tốt nhất.
    Các tham số cần tune: alpha, score_threshold, top_k_stage1.
    """
    print("\n" + "=" * 60)
